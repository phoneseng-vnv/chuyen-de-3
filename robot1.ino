...
void setup() {
  ...
  for(int i=0; i<6; i++) { 
    ...
    // Chữ Z bắt đầu rơi từ phía trên màn hình (y < 0)
...
void checkTouch() {
  ...
  if (currentBtnState == LOW && lastBtnState == HIGH) {
    ...
    if (robotState == SLEEP || robotState == DROWSY) {
      ...
      stateTimer = millis();
      if(emotionType == 3) {
        for(int i=0; i<6; i++) { hearts[i].y = random(-64, 0); }
      }
    }
    lastActivityTime = millis();
  }
...
void drawSleepEyes() {
  ...
  // Hiệu ứng chữ z rơi từ đỉnh màn hình
  display.setTextSize(1);
  for(int i=0; i<6; i++) {
    ...
    if(sleepZ[i].y > 64) {
      sleepZ[i].y = -10; // Reset lên trên đỉnh màn hình
      sleepZ[i].x = random(5, 120);
    }
  }
...
void drawEmotions() {
  switch (emotionType) {
    case 0: { // VUI
      ...
      if (millis() % 2000 < 150) {
        display.fillRect(35, 35 + bounce, 22, 4, WHITE);
        display.fillRect(71, 35 + bounce, 22, 4, WHITE);
      } else {
        display.drawCircleHelper(45, 38 + bounce, 18, 1, WHITE); 
        display.drawCircleHelper(45, 39 + bounce, 18, 1, WHITE);
        display.drawCircleHelper(83, 38 + bounce, 18, 1, WHITE);
        display.drawCircleHelper(83, 39 + bounce, 18, 1, WHITE);
      }
    ...
    case 2: { // BUỒN
      if (millis() % 5000 < 400) { 
        display.fillRect(35, 35, 22, 2, WHITE); 
        display.fillRect(71, 35, 22, 2, WHITE);
      } else {
        display.fillRoundRect(35, 30, 22, 12, 4, WHITE); 
        display.fillRoundRect(71, 30, 22, 12, 4, WHITE);
      }
      display.drawFastHLine(54, 55, 20, WHITE);
      for(int i=0; i<2; i++) {
    ...
    case 3: { // YÊU + TIM ZOOM ĐẶC
      unsigned long elapsed = millis() - stateTimer;
      if (elapsed < 3000) { 
        display.fillCircle(38, 30, 7, WHITE); display.fillCircle(50, 30, 7, WHITE);
        display.fillTriangle(31, 33, 57, 33, 44, 50, WHITE);
        display.fillCircle(78, 30, 7, WHITE); display.fillCircle(90, 30, 7, WHITE);
        display.fillTriangle(71, 33, 97, 33, 84, 50, WHITE);
        for(int i=0; i<6; i++) {
          drawHeart(hearts[i].x, hearts[i].y, 4);
          hearts[i].y += hearts[i].speed;
          if(hearts[i].y > 64) { hearts[i].y = -5; hearts[i].x = random(10, 118); }
        }
      } else { 
        int heartSize = (elapsed - 3000) / 40; 
        if (heartSize > 70) {
          heartSize = 70 + (millis() % 200 < 100 ? 5 : 0);
        }
        drawHeart(64, 32 - (heartSize/4), heartSize); // Luôn là màu trắng đặc
      }
      break;
    }
  }
